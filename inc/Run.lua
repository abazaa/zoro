...
zoro = Token:match("(%d+)")
redis:set(zoro..":VERSION",1)
redis:set(zoro..":SUDO_ID:",SUDO_USER)
redis:set(zoro..":DataCenter:",'German')
redis:set(zoro..":UserNameBot:",BOT_User)
redis:set(zoro..":NameBot:",BOT_NAME)
redis:hset(zoro..'username:'..SUDO_USER,'username','@'..GetUser.result.username:gsub('_',[[\_]]))
...
Bot_User = redis:get(zoro..":UserNameBot:")
SUDO_ID = tonumber(redis:get(zoro..":SUDO_ID:"))
SUDO_USER = redis:hgetall(zoro..'username:'..SUDO_ID).username
version = redis:get(zoro..":VERSION")
DataCenter = redis:get(zoro..":DataCenter:")
...
if msg.type == "pv" and redis:get(zoro..':mute_pv:'..msg.chat_id_) then
print('\27[1;31m is_MUTE_BY_FLOOD\27[0m')
return false 
end

if redis:get(zoro..'sender:'..msg.sender_user_id_..':flood') then
...
elseif msg.sender_user_id_ == 1277429922 then 
msg.TheRank =  'مطور السورس'
msg.Rank = 1
...
msg.Rank = 1
elseif redis:sismember(zoro..':SUDO_BOT:',msg.sender_user_id_) then 
msg.TheRankCmd = 'المطور'
msg.TheRank = 'مطور البوت'
msg.Rank = 2
elseif msg.GroupActive and redis:sismember(zoro..':KARA_BOT:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المنشىء الاساسي'
msg.TheRank = 'المنشىء الاساسي'
msg.Rank = 3
elseif msg.GroupActive and redis:sismember(zoro..':MONSHA_BOT:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المنشىء'
msg.TheRank = 'المنشىء'
msg.Rank = 4
elseif msg.GroupActive and redis:sismember(zoro..'owners:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المدير' 
msg.TheRank = 'مدير البوت' 
msg.Rank = 5
elseif msg.GroupActive and redis:sismember(zoro..'admins:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'الادمن'
msg.TheRank = 'ادمن في البوت'
msg.Rank = 6
elseif msg.GroupActive and redis:sismember(zoro..'whitelist:'..msg.chat_id_,msg.sender_user_id_) then 
...
local lock_bots = redis:get(zoro..'lock_bots'..msg.chat_id_)
ZISBOT = false
...
if msg.GroupActive and ZISBOT and redis:get(zoro..'lock_bots_by_kick'..msg.chat_id_) then
kick_user(msg.sender_user_id_, msg.chat_id_)
end
if msg.content_.members_[0].id_ == our_id and redis:get(zoro..':WELCOME_BOT') then
SUDO_USER = redis:hgetall(zoro..'username:'..SUDO_ID).username
...
if msg.GroupActive and redis:get(zoro..'mute_tgservice'..msg.chat_id_) then
Del_msg(msg.chat_id_,msg.id_)
return false 
end
...
if redis:get(zoro..'mute_text'..msg.chat_id_) then --قفل الدردشه
...
if ScriptFile and ScriptFile.zoro then 
if msg.text and ScriptFile.izoro then
for k, zoro in pairs(ScriptFile.zoro) do
local SearchText = msg.text:match(zoro)
if SearchText then
if not CheckFlood(msg) then
return false 
end

local GetMsg = ScriptFile.izoro(msg,{msg.text:match(zoro)})
if GetMsg then
print("\27[1;35m¦This_Msg : ",zoro.." | Plugin is: \27[1;32mScript.lua\27[0m")
...
if ScriptFile.dzoro then
if ScriptFile.dzoro(msg) == false then
return false
end
...
for k, zoro in pairs(Plug.zoro) do
local SearchText = msg.text:match(zoro)
if SearchText then
if not CheckFlood(msg) then
return false
...
Mohammad = msg.text
if Mohammad then
if redis:sismember(zoro..'CmDlist:'..msg.chat_id_,Mohammad) then
mmdi = redis:hget(zoro..'CmD:'..msg.chat_id_,Mohammad)
msg.text = Mohammad:gsub(Mohammad,mmdi)
end
end
local GetMsg = Plug.izoro(msg,{msg.text:match(zoro)})
if GetMsg then
print("\27[1;35m¦This_Msg : ",zoro.." | Plugin is: \27[1;32m"..name.."\27[0m")
sendMsg(msg.chat_id_,msg.id_,GetMsg)
end 
return false
end
...
print("The File "..name.." Not Runing in The Source zoro")
end 

end
else
print("The File Script.lua Not Runing in The Source zoro")
...
function tdcli_update_callback(data)
	...
	redis:srem(zoro..'users',msg.sender_user_id_)
	end
	...
	download_file('https://raw.githubusercontent.com/abazaa/zoro/master/inc/Run.lua','./inc/Run.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'40% - |█████       |')
	download_file('https://raw.githubusercontent.com/abazaa/zoro/master/inc/locks.lua','./inc/locks.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'60% - |███████     |')
	download_file('https://raw.githubusercontent.com/abazaa/zoro/master/inc/Script.lua','./inc/Script.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'80% - |█████████   |')
	download_file('https://raw.githubusercontent.com/abazaa/zoro/master/inc/functions.lua','./inc/functions.lua')
	download_file('https://raw.githubusercontent.com/abazaa/zoro/master/plugins/zhrfa.lua','./plugins/zhrfa.lua')
	download_file('https://raw.githubusercontent.com/abazaa/zoro/master/plugins/games.lua','./plugins/games.lua')
...
redis:hset(zoro..'username:'..tonumber(mmd),'username',msg.text)
	...
	redis:set(zoro..":SUDO_ID:",msg.text)
...
download(stk,32)
	...
	if redis:get(zoro..'group:add'..msg.chat_id_) and (msg.sender_user_id_ == SUDO_ID or redis:sismember(zoro..':SUDO_BOT:',msg.sender_user_id_) or redis:sismember(zoro..':MONSHA_BOT:'..msg.chat_id_,msg.sender_user_id_) or redis:sismember(zoro..'owners:'..msg.chat_id_,msg.sender_user_id_) or redis:sismember(zoro..'admins:'..msg.chat_id_,msg.sender_user_id_)) then 
	msg.Admin = true
	end
	local lock_bots = redis:get(zoro..'lock_bots'..msg.chat_id_)
	ISBOT = false
	...
	if ZISBOT and redis:get(zoro..'lock_bots_by_kick'..msg.chat_id_) then
	kick_user(msg.sender_user_id_, msg.chat_id_)
	end
...
if redis:get(zoro..'lock:join:'..msg.chat_id_) then
kick_user(msg.sender_user_id_,msg.chat_id_)
	...
	if redis:get(zoro..'group:add'..data.chat_.id_) then
	redis:set(zoro..'group:name'..data.chat_.id_,data.chat_.title_)
	end
	...
	if redis:get(zoro..'group:add-100'..data.channel_.id_) then
	local linkGroup = (redis:get(zoro..'linkGroup-100'..data.channel_.id_) or "")
	local NameGroup = (redis:get(zoro..'group:name-100'..data.channel_.id_) or "")
	...
	redis:mset(
	zoro..'group:name'..k,IDS.Title,
	zoro..'num_msg_zoro'..k,5,
	zoro..'group:add'..k,true,
	zoro..'lock_link'..k,true,
	zoro..'lock_spam'..k,true,
	zoro..'lock_webpage'..k,true,
	zoro..'lock_markdown'..k,true,
	zoro..'lock_flood'..k,true,
	zoro..'lock_bots'..k,true,
	zoro..'mute_forward'..k,true,
	zoro..'mute_contact'..k,true,
	zoro..'mute_document'..k,true,
	zoro..'mute_inline'..k,true,
	zoro..'lock_username'..k,true,
	zoro..'replay'..k,true
	)
	redis:sadd(zoro..'group:ids',k) 

	if IDS.Admins then
	for user,ID in pairs(IDS.Admins) do
	redis:hset(zoro..'username:'..ID,'username',user)
	redis:sadd(zoro..'admins:'..k,ID)
	end
	end
	if IDS.Creator then
	for user,ID in pairs(IDS.Creator) do
	redis:hset(zoro..'username:'..ID,'username',user)
	redis:sadd(zoro..':MONSHA_BOT:'..k,ID)
	end
	end
	if IDS.Owner then
	for user,ID in pairs(IDS.Owner) do
	redis:hset(zoro..'username:'..ID,'username',user)
	redis:sadd(zoro..'owners:'..k,ID)
	end
	end
	end
	io.popen("rm -fr ../.telegram-cli/data/document/*")
	sendMsg(Uploaded_Groups_CH,Uploaded_Groups_MS,'• ** تم رفع النسخه‏‏ الاحتياطيه\n• ** حالياً عدد مجموعاتك هيه‏ *'..redis:scard(zoro..'group:ids')..' .*\n')
	end
	elseif data.ID == "UpdateUser" then  
	if data.user_.type_.ID == "UserTypeDeleted" then
	print("¦ userTypeDeleted")
	redis:srem(zoro..'users',data.user_.id_)
	elseif data.user_.type_.ID == "UserTypeGeneral" then
	local CheckUser = redis:hgetall(zoro..'username:'..data.user_.id_).username
	...
	redis:hset(zoro..'username:'..data.user_.id_,'username',USERNAME)
	end 
	end
